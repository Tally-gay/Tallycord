...
import { Values } from "ts-pattern/dist/types/helpers";
...
type State = Values<typeof State>[number];
...
function tick() {
    const now = Date.now();
    cats.forEach(cat => {
        // if (!cat.state) cat.state = State.Wander;
        ...
        if (settings.store.devMode && cat.oneko.element) {
            cat.oneko.element.setAttribute("data-dev", `${cat.personality} | ${State[cat.state]} (${timeSec}s)`);
        } else {
